...
@Component({
  selector: 'page-to-read',
  templateUrl: 'to-read.html',
})
export class ToReadPage {
  ...
  getDidNotReadBooks() {
    this.databaseprovider.getDidNotReadBooks().then(data => {
      this.books = data;
  ...
  searchByName(event) {
    ...
    if (searchText != "") {
      this.databaseprovider.getDidNotReadBooksByName(searchText).then(data => {
        this.books = data;
        for (let book of this.books) {
          var awardArray: any[] = [];
          var awards = book.awards.split("/");
  ...
  loadTestData() {
    ...
    this.books = [
      ...
      { row_id: 3, id: 'T/002/002', title: 'පිටුවහල් සිත්තරී 2', title_english: 'Pituwahal Siththari 2', authour: 'මුදිතා හේරත්', note: 'එළි පෙහෙළි ලෝකයක කැලෑව සුවිශේෂිය. ප්‍රඥාවන්තිය කැලෑවේ සැඟවී සිටියි. කැළෑව එළි පෙහෙළි කළ විටද ඇය දැකිය නොහැක. මන්ද යත් සුවිශේෂත්වය එළිපෙහෙළි වී සාමාන්‍ය බවට පත්ව ඇති හෙයිනි. සාමාන්‍යජීවිතය තුල ඈ උමතු කාන්තාවකි.සාමාන්‍ය ජීවිතයෙ සැරිසරන්නන්ට මේ උමතු ගැහැනිය නිතර දෙවේලේ හමු වේ.එහෙත් සුවිශේෂත්වය තුළ ඈ ප්‍රඥාවන්තියකි. මෙය උමතු ගැහැනියක පිලිබද කතාවක් නොවේ. සාමාන්‍ය ගැහැණුන් දෙදෙනෙකු පිලිබද කතවකි. ඔබේ සිත් මානය අනුව ඔවුනට උමතු බව ආරෝපණය වීම හෝ නොවිම සිදු වේ.', category: 'පරිවර්තන', fpublished_date: '2011-02-08', published_date: '2012-09-25', publisher: 'සරසවි ප්‍රකාශන', series: 7, pages: 213, isbn: 9789556712543, price: 550, volume: 5, icon: 'ios-bookmarks-outline', awards: '{}', is_read: 2, lend_name: '', lend_date: '', special_note: '' }
    ];
    for (let book of this.books) {
      var awardArray: any[] = [];
      var awards = book.awards.split("/");
    ...
    this.books = filter(this.books, (value)=> {
      return value.is_read != 1;
    })
